...
def save_project(project_id, shopify_shop, image, attributes, applied_attribute):
    ...
    try:
        ...
        if existing_project:
            ...
            query = '''
                INSERT INTO projects (shopify_shop, image, attributes, applied_attribute, created_at)
                VALUES (?, ?, ?, ?, ?)
            '''
            params = (shopify_shop, image, json.dumps(attributes), applied_attribute, datetime.now().isoformat())
...
# Initialize the database when the module is executed directly
